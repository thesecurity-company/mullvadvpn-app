...
class AutomaticKeyRotationManager {
        ...
        return rest.replaceWireguardKey().dataTask(payload: payload) { (result) in
            self.dispatchQueue.async {
                let updateResult = result.mapError { (error) -> Error in
                    ...
                    return self.updateTunnelSettings(privateKey: newPrivateKeyMetadata, addresses: addresses)
                }
    ...
    private func updateTunnelSettings(privateKey: PrivateKeyMetadata, addresses: WireguardAssociatedAddresses) -> Result<TunnelSettings, Error> {
